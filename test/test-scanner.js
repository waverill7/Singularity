...
describe('The scanner', function () {
  ...
  it('distinguishes reserved words and identifiers', function (done) {
    scan('test/data/token-tests/words', function (tokens) {
      ...
      i(tokens[23]).should.equal(i({kind:'or',lexeme:'or',line:1,col:75}));
      i(tokens[24]).should.equal(i({kind:'and',lexeme:'and',line:1,col:78}));
      i(tokens[25]).should.equal(i({kind:'not',lexeme:'not',line:1,col:82}));
      i(tokens[26]).should.equal(i({kind:'true',lexeme:'true',line:1,col:86}));
      i(tokens[27]).should.equal(i({kind:'false',lexeme:'false',line:1,col:91}));
      i(tokens[28]).should.equal(i({kind:'ID',lexeme:'whilexy',line:1,col:97}));
      i(tokens[29]).should.equal(i({kind:'ID',lexeme:'whil',line:1,col:105}));
      i(tokens[30]).should.equal(i({kind:'ID',lexeme:'ore',line:1,col:110}));
      i(tokens[31]).should.equal(i({kind:'ID',lexeme:'globallocal',line:1,col:114}));
      done();
    });
  });
